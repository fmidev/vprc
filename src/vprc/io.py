...
if __name__ == '__main__':
    ...
    test_file = Path('~/data/prodx/202508241100_KAN.VVP_40.txt').expanduser()

    if test_file.exists():
        print("="*60)
        print("TEST 1: Step-by-step API (parse_vvp_file + to_xarray)")
        print("="*60)

        header, df = parse_vvp_file(test_file)
        print(f"\n{header}")
        print(f"\nDataFrame shape: {df.shape}")
        print(f"Height range: {df['height'].min()} - {df['height'].max()} m")

        radar_meta = {
            'antenna_height_km': 0.174,
            'lowest_level_offset_m': 126,
            'freezing_level_m': None  # To be filled from external file
        }
        ds = vvp_dataframe_to_xarray(df, header, radar_meta, test_file)
        print(f"\nxarray Dataset:\n{ds}\n")

        # Test high-level wrapper
        print("="*60)
        print("TEST 2: High-level API (parse_vvp_to_xarray)")
        print("="*60)

        header2, ds2 = parse_vvp_to_xarray(test_file, radar_meta)
        print(f"\n{header2}")
        print(f"\nxarray Dataset:\n{ds2}\n")

        # Quick sanity check
        print("="*60)
        print("Sanity check: Accessing data")
        print("="*60)
        print(f"\nReflectivity at 1500m: {ds2['lin_dbz'].sel(height=1500).values:.1f} dBZ")
        print(f"Sample counts at surface: {ds2['zcount'].sel(height=100).values}")
        print(f"\nRadar: {ds2.attrs['radar']}, Time: {ds2.attrs['timestamp']}")
